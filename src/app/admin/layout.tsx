...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-100">
      ...
      <nav className="border-b border-stone-800 bg-stone-900/60">
        <div className="mx-auto flex max-w-7xl items-center gap-2 px-4 py-2 text-sm">
          <Link href="/admin" className="rounded-lg px-3 py-1.5 text-stone-400 transition hover:bg-stone-800 hover:text-white">Ringkasan</Link>
          <Link href="/admin/undangan" className="rounded-lg px-3 py-1.5 text-stone-400 transition hover:bg-stone-800 hover:text-white">Undangan</Link>
        </div>
      </nav>
